...
void sendIP(char faceRecognized) {
    // Kiểm tra địa chỉ ESP-NOW không NULL
    if (esp8266Address == NULL) {
        Serial.println("Error: ESP-NOW address is NULL");
        return;
    }

    // Lấy địa chỉ IP
    IPAddress ip = WiFi.localIP();
    if (ip == INADDR_NONE) { // Kiểm tra nếu IP là 0.0.0.0
        Serial.println("Error: Invalid IP address");
        return;
    }

    // Định dạng IP và cập nhật trạng thái
    snprintf(myData.ipAddress, sizeof(myData.ipAddress), "%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);
    myData.faceRecognized = faceRecognized;

    // Gửi dữ liệu qua ESP-NOW
    esp_err_t result = esp_now_send(esp8266Address, (uint8_t *)&myData, sizeof(myData));

    // Xử lý kết quả gửi
    if (result == ESP_OK) {
        Serial.print("IP Address sent: ");
        Serial.print(myData.ipAddress);
        Serial.print(", Face Recognized: ");
        Serial.println(myData.faceRecognized ? "Yes" : "No");
    } else {
        Serial.print("Error sending the data. ESP-NOW result: ");
